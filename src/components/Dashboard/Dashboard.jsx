...
export default function Dashboard() {
  ...
  const { player, quests, loseXp, setStreak, setLastLoginDate } = usePlayer();
  const today = getTodayString();
  const remainingQuests = quests.filter((q) => q.status !== "Completed");

  const [penaltyNotice, setPenaltyNotice] = useState(null);
  const [playerName, setPlayerName] = useState(
    () => localStorage.getItem("playerName") || "Sung Jin-Woo"
  );
  const [playerPhoto, setPlayerPhoto] = useState(
    () => localStorage.getItem("playerPhoto") || null
  );
  ...
  return (
    <div className="dashboard">
      ...
      <div className="dashboard-column">
        <h1 className="section-heading heading-center">Statistics</h1>
        <Statistics />
      </div>
